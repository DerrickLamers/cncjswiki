...
CNCjs is a web-based interface for CNC controllers running [Grbl](https://github.com/grbl/grbl), [Marlin](https://github.com/MarlinFirmware/Marlin), [Smoothieware](https://github.com/Smoothieware/Smoothieware), or [TinyG](https://github.com/synthetos/TinyG).  Such CNC controllers are often implemented as an Arduino plus stepper motor driver hardware, fed with GCode from a serial port.  They can be used for milling, laser cutting/engraving, 3D printing, and other kinds of machine control.

CNCjs consists of a server component and a user interface.  The server talks to the CNC controller (e.g. the Arduino) over a serial connection using a USB serial port, a Bluetooth serial module, or a Serial-to-WiFi module like [XBee](https://www.arduino.cc/en/Guide/ArduinoWirelessShieldS2) or [USR-WIFI232-T](https://gist.github.com/ajfisher/1fdbcbbf96b7f2ba73cd).  The server component also behaves like a web server, so it can be controlled from a web browser like Chrome.  The server can run on a small computer like a [Raspberry Pi](https://www.raspberrypi.org/) or on a laptop computer like a PC or Mac.  The server computer may have a graphics display, but does not require one.  The server is written in the JavaScript programming language, using the Node.js framework.

The CNCjs user interface consists of a collection of JavaScript code that runs inside a web browser.  It lets the user set up and control the CNC machine, doing things like jogging, setting machine positions, and controlling the execution of GCode programs.  That web browser can run on the same computer that is running the server component, or it can run on any computer that can reach the server computer over a network.  That browser can also run on portable devices like tablet computers and smartphones.  Multiple instances of the user interface can run at the same time, perhaps on different computers, all talking to the same server.

The JavaScript code that implements the user interface is usually resident on the server computer, "served up" to the browser or browsers running on the same or different computers.  This makes it possible to run CNCjs on an isolated network that is not connected to the Internet.  It is also possible for portions of the user interface code to reside out on the Internet, but that is not required.

There is also a CNCjs "desktop app", which appears as a single component, but in reality it is based on the same server/web-browser design, with the two components bundled together into a single executable image, with both components running on the same computer.
